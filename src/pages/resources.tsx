...
class Resources extends Component<ResourcesProps> {
  ...
  filterResources = async (event: FilterEvent) => {
    ...
    this.changeUrl(resourcesAndFilters);
  ...
  getFilteredResources = (
    ...
    return theResources.filter(resource => {
      ...
      const isSearch =
        theSearchFilter === BLANK 
        ? true 
        : name.includes(filter) || resource.tags.some(val => val.toLowerCase().includes(filter));

      return isPrice && isResource && isSearch;
    });
  };

  changeUrl({ category, price, search }: ResourcesAndFilters) {
    ...
    if (price && price !== "Show All") {
      queryParams.set("Filterbyprice", price);
    }
    ...
    void this.props.router.push(url, undefined, { shallow: true });
  }

  componentDidMount() {
    const categoriesLower = categories.map(c => c.toLowerCase());
    const { category, Filterbyprice, search } = getQueryParams(this.props.router);
    ...
    if (Filterbyprice) {
      this.setState({ filterPrice: Filterbyprice }, () => {
        void this.filterResources({
          target: {
            value: Filterbyprice,
            name: "price"
